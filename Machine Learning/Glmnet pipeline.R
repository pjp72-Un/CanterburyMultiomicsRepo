...
setwd("C:/PhD/Canterbury dataset/crossomic")

# USER CONFIG

base_out_dir   <- "glmnet_group"
block_dir       <- "aligned_blocks"
aligned_rds    <- file.path(block_dir, "aligned_blocks_genus_30_status.rds")
out_dir        <- file.path(base_out_dir, "genus_30_status_therun")

# Outer CV
outer_folds    <- 4          # recommend 3 or 4 with n=28 and 8 treated
outer_repeats  <- 5000        # increase for stability
outer_seed     <- 1
...
fusion_policy  <- "always" # "always" or "metabolome_primary"
...
driver_prop_min <- 0.5   # drivers = features selected in >= this fraction of refits
driver_top_n <- 10      # safety cap on the number of drivers per block
...
if (nrow(oof_df) > 0 && length(classes) == 2) {
  ...
  if (!is.null(prob_micro_vals)) {
    ...
    if (length(scores_pos) > 0 && length(scores_neg) > 0) {
      ...
      plot(pr_micro, color = "#0072B2", legend = FALSE)
      dev.off()
    }
  }
  ...
  if (!is.null(prob_met_vals)) {
    ...
    if (length(scores_pos) > 0 && length(scores_neg) > 0) {
      ...
      plot(pr_met, color = "#0072B2", legend = FALSE)
      dev.off()
    }
  }
  ...
  if (!is.null(prob_fused_vals)) {
    ...
    if (length(scores_pos) > 0 && length(scores_neg) > 0) {
      ...
      plot(pr_fused, color = "#0072B2", legend = FALSE)
      dev.off()
    }
  }
...
make_driver_table <- function(stab_df, block_name, top_n, prop_min) {
  ...
  # drivers = features selected in >= prop_min of refits, capped at top_n
  out <- out[out$prop_selected >= prop_min, , drop = FALSE]
  head(out, top_n)
}

drivers_micro <- make_driver_table(micro_stab, "Microbiome", driver_top_n, driver_prop_min)
drivers_metab <- make_driver_table(metab_stab, "Metabolome", driver_top_n, driver_prop_min)
