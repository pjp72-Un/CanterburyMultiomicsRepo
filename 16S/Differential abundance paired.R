...
untreated_treated_ps1 <- readRDS("C:/PhD/Sequencing Results/Files/Cant_5%_untreated_treated.rds")

outdir <- "C:/PhD/Canterbury Dataset/16S/ancom/untreated_treated"
dir.create(outdir, showWarnings = FALSE)

# ----------------------------
set.seed(7)

ps <- untreated_treated_ps1
...
#Set factor level
sd <- sample_data(ps)
sd$GFD <- factor(sd$GFD, levels = c(FALSE, TRUE))
sample_data(ps) <- sd
...
lfc_col <- grep("^lfc_.*GFD", names(comb_res), value = TRUE)[1]
se_col  <- grep("^se_.*GFD",  names(comb_res), value = TRUE)[1]
...
## Grouping
diag_vec     <- sample_data(rel_abund)$GFD
